...
class BasisLunarLanderNetworkWrapper(torch.nn.Module):
    """
    Wrapper for lunarlander basis network
    """
    def __init__(self, input_size, hidden_sizes, basis="equivariant",
                 gain_type="xavier"):
        super().__init__()
        in_group = get_lunarlander_state_group_representations()
        out_group = get_lunarlander_action_group_representations()

        repr_in = MatrixRepresentation(in_group, out_group)
        repr_out = MatrixRepresentation(out_group, out_group)


        self.network = BasisLunarLanderNetwork(repr_in, repr_out, input_size,
...
class SingleBasisLunarLanderLayer(torch.nn.Module):
    """
    Single layer for lunarlander symmetries
    """
...
class BasisLunarLanderLayer(torch.nn.Module):
    """
    """
    def __init__(self, input_size, output_size, basis="equivariant",
                 out="equivariant", gain_type="xavier"):
        """
        Wrapper for single layer with lunarlander symmetries, allows
        ...
        if out == "equivariant":
            ...
            in_group = get_lunarlander_action_group_representations()
            out_group = get_lunarlander_invariants()
