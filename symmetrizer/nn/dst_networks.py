...
class BasisDstNetworkWrapper(torch.nn.Module):
    """
    Wrapper for dst basis network
    """
    def __init__(self, input_size, hidden_sizes, basis="equivariant",
                 gain_type="xavier"):
        super().__init__()
        in_group = get_dst_state_group_representations()
        out_group = get_dst_action_group_representations()

        repr_in = MatrixRepresentation(in_group, out_group)
        repr_out = MatrixRepresentation(out_group, out_group)


        self.network = BasisDstNetwork(repr_in, repr_out, input_size,
...
class SingleBasisDstLayer(torch.nn.Module):
    """
    Single layer for dst symmetries
    """
...
class BasisDstLayer(torch.nn.Module):
    """
    """
    def __init__(self, input_size, output_size, basis="equivariant",
                 out="equivariant", gain_type="xavier"):
        """
        Wrapper for single layer with dst symmetries, allows
        ...
        if out == "equivariant":
            ...
            in_group = get_dst_action_group_representations()
            out_group = get_dst_invariants()
