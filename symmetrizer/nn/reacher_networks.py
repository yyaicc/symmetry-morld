...
class BasisReacherNetworkWrapper(torch.nn.Module):
    """
    Wrapper for reacher basis network
    """
    def __init__(self, input_size, hidden_sizes, basis="equivariant",
                 gain_type="xavier"):
        super().__init__()
        in_group = get_reacher_state_group_representations()
        out_group = get_reacher_action_group_representations()

        repr_in = MatrixRepresentation(in_group, out_group)
        repr_out = MatrixRepresentation(out_group, out_group)


        self.network = BasisReacherNetwork(repr_in, repr_out, input_size,
...
class SingleBasisReacherLayer(torch.nn.Module):
    """
    Single layer for reacher symmetries
    """
...
class BasisReacherLayer(torch.nn.Module):
    """
    """
    def __init__(self, input_size, output_size, basis="equivariant",
                 out="equivariant", gain_type="xavier"):
        """
        Wrapper for single layer with reacher symmetries, allows
        ...
        if out == "equivariant":
            ...
            in_group = get_reacher_action_group_representations()
            out_group = get_reacher_invariants()
